...
function viewMore(button){
    ...
    let ajaxUrl = base_url+'ConceptualResult/getConceptualResultById/' + idConceptualResult;
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                document.querySelector("#titleCRModal").innerHTML = objData.msg.nombre;
                document.querySelector("#descriptionCRModal").innerHTML = objData.msg.detalle;
            } else {
                swal("Error", objData.msg, "error");
            }
        } 
    }
    $('#conceptualResultModal').modal('show');
}
...
function searchData(query){
    $.ajax({
        url: " "+base_url+"ConceptualResult/SearchBarConceptual",
        type: 'POST',
        dataType: 'html',
        data: {query: query},
    })
...
function changePage(button){
    ...
    $.ajax({
        url: " "+base_url+"ConceptualResult/getConceptualResult",
        type: 'POST',
        dataType: 'html',
