...
function viewMore(button){
    ...
    let ajaxUrl = base_url+'ObjectResult/getObjectResultById/' + idObjectResult;
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                document.querySelector("#titleORModal").innerHTML = objData.msg.descripcion;
                document.querySelector("#descriptionORModal").innerHTML = objData.msg.detalle;
            } else {
                swal("Error", objData.msg, "error");
            }
        } 
    }
    $('#objectResultModal').modal('show');
}
...
function searchData(query){
    $.ajax({
        url: " "+base_url+"ObjectResult/SearchBarObject",
        type: 'POST',
        dataType: 'html',
        data: {query: query},
    })
...
function changePage(button){
    ...
    $.ajax({
        url: " "+base_url+"ObjectResult/getObjectResult",
        type: 'POST',
        dataType: 'html',
