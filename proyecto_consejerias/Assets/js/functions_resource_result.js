...
function viewMore(button){
    let idResourceResult = button.getAttribute('rr');
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url+'ResourceResult/getResourceResultById/' + idResourceResult;
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                document.querySelector("#titleRRModal").innerHTML = objData.msg.descripcion;
                document.querySelector("#descriptionRRModal").innerHTML = objData.msg.detalle;
            } else {
                swal("Error", objData.msg, "error");
            }
        } 
    }
    $('#resourceResultModal').modal('show');
}
...
function searchData(query){
    $.ajax({
        url: " "+base_url+"ResourceResult/SearchBarResource",
        type: 'POST',
        dataType: 'html',
        data: {query: query},
    })
...
function changePage(button){
    ...
    $.ajax({
        url: " "+base_url+"ResourceResult/getResourceResult",
        type: 'POST',
        dataType: 'html',
